...
struct PicksView: View {
    ...
    private var emptyStateView: some View {
        VStack(spacing: DS.Spacing.s4) {
            Image(systemName: SFSymbol.picksDeadline)
                .font(.system(size: 56))
                .foregroundStyle(Color.DS.Neutral.n300)
        ...
        ScrollView {
            VStack(spacing: DS.Spacing.s3) {
                ...
                if let deadline = viewModel.deadlineText {
                    HStack {
                        Image(systemName: SFSymbol.timeRemaining)
    ...
    private func currentPickBanner(pick: Pick) -> some View {
        HStack {
            Image(systemName: pick.isLocked ? SFSymbol.lockedPick : SFSymbol.success)
    ...
    private func errorBanner(message: String) -> some View {
        HStack {
            Image(systemName: SFSymbol.errorFill)
